...
import Prelude (Applicative (pure), Eq, Integer, Num ((*)), Ord, Show, ($), (.))

-- | @since 1.3.0
newtype Discrete (tag :: k) = Discrete (Tagged tag Integer)
  deriving stock
    ( -- | @since 1.3.0
      Generic
    )
...
-- | @since 1.3.0
instance PUnsafeLiftDecl (PDiscrete t) where
  type PLifted (PDiscrete t) = Discrete t

-- | @since 1.3.0
deriving via
  (DerivePConstantViaNewtype (Discrete t) (PDiscrete t) (PTagged t PInteger))
  instance
    (PConstantDecl (Discrete t))
