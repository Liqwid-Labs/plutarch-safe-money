...
@echo "  ci                  -- Run the full CI (hydra) derivation"
...
-killall -q hoogle
hoogle generate --local=haddock --database=plutarch-safe-money.hoo
hoogle server --local --database=plutarch-safe-money.hoo -p 8070 >> /dev/null && echo $! > ./.hoogle.pid &
...
while sleep 1;																				\
do	                                                  \
  find src testlib test plutarch-safe-money.cabal |   \
...
cabal v2-repl $(GHC_FLAGS) plutarch-safe-money
